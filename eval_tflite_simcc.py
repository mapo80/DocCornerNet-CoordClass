...
def iterate_hf_data(hf_dataset_path: str, split: str, img_size: int, input_norm: str, batch_size: int = 32):
    """Generator that yields batches from HuggingFace parquet dataset."""
    ...
    for pf in parquet_files:
        ...
        is_negative_col = table.column("is_negative").to_pylist()

        coords_cols = {
            "tl_x": table.column("corner_tl_x").to_pylist(),
            "tl_y": table.column("corner_tl_y").to_pylist(),
            "tr_x": table.column("corner_tr_x").to_pylist(),
            "tr_y": table.column("corner_tr_y").to_pylist(),
            "br_x": table.column("corner_br_x").to_pylist(),
            "br_y": table.column("corner_br_y").to_pylist(),
            "bl_x": table.column("corner_bl_x").to_pylist(),
            "bl_y": table.column("corner_bl_y").to_pylist(),
        }
        ...
        for i in range(len(images_col)):
            ...
            elif input_norm == "zero_one":
                img_arr = img_arr / 255.0
            # else raw255: keep as-is

            batch_images.append(img_arr)

            coords = np.array([
                coords_cols["tl_x"][i], coords_cols["tl_y"][i],
                coords_cols["tr_x"][i], coords_cols["tr_y"][i],
                coords_cols["br_x"][i], coords_cols["br_y"][i],
                coords_cols["bl_x"][i], coords_cols["bl_y"][i],
            ], dtype=np.float32)
            batch_coords.append(coords)

            is_negative = is_negative_col[i]
            has_doc = 0.0 if is_negative else 1.0
            batch_has_doc.append(has_doc)

            if len(batch_images) >= batch_size:
                yield (
                    np.array(batch_images),
                    np.array(batch_coords),
                    np.array(batch_has_doc)
                )
                batch_images = []
                batch_coords = []
                batch_has_doc = []
...
def parse_args():
    ...
    p.add_argument("--data_root", type=str, default=None, help="Dataset root (legacy format)")
    p.add_argument("--hf_dataset", type=str, default=None, help="HuggingFace parquet dataset path")
    p.add_argument("--split", type=str, default="val", help="Split name")
    p.add_argument(
        "--input_norm",
        type=str,
        default="imagenet",
...
def evaluate_model(
    ...
    # Choose data source
    use_hf = hf_dataset is not None
    if use_hf:
        data_iter = iterate_hf_data(hf_dataset, split, img_size, input_norm, batch_size)
    else:
        if data_root is None:
            raise ValueError("Either --data_root or --hf_dataset must be specified")
    ...
    seen = 0
    for batch_data in tqdm(data_iter, desc=f"TFLite eval (SimCC logits) ({Path(tflite_path).name})"):
        if use_hf:
            x, coords_gt, has_doc = batch_data
        else:
